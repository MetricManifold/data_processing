...
#[macro_export]
macro_rules! v2_metric {
    ($obs:ty, $extract:expr) => {
        Box::new(|bag: &$crate::analysis::v2::observable::ObservableBag| {
            bag.get::<$obs>().map($extract)
        }) as $crate::analysis::v2::aggregate::MetricExtractor
    };
}
...
mod tests {
    ...
    use crate::analysis::v2::observables::msd::{Msd, MsdOutput};
    ...
    fn mean_stderr_computes_correctly() {
        ...
        let metrics: &[(&str, MetricExtractor)] = &[(
            "msd_lag1",
            v2_metric!(Msd, |out| out.cell0_values[0]),
        )];
