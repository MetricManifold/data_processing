...
/// Single-run L_n(t) renderer. `decimate_max = Some(N)` thins the
/// trace to at most ~N points via step_by; default None preserves
/// every frame for full-fidelity rendering.
#[derive(Default)]
pub struct LnTimeseriesSingle {
    pub decimate_max: Option<usize>,
}

impl<'a, 'b> Panel<'a, 'b> for LnTimeseriesSingle {
    type Data = SingleRunData<'a>;

    fn id(&self) -> &'static str {
        "ln_timeseries_single"
    }
    ...
    fn render(
        ...
        let ln = data
            ...
            .bag
            .get::<LnPerimeter>()
            .ok_or_else(|| anyhow!("ln_timeseries_single: missing ln_perimeter"))?;

        let valid: Vec<f64> = ln.series.iter().copied().filter(|v| v.is_finite()).collect();
        ...
        let x_max = ln.t_tau.last().copied().unwrap_or(1.0);
        ...
        let step = self.decimate_max
            .map(|m| (ln.t_tau.len() / m.max(1)).max(1))
            .unwrap_or(1);
        chart
            .draw_series(LineSeries::new(
                ln.t_tau
                    .iter()
                    .step_by(step)
                    .zip(ln.series.iter().step_by(step))
                    .map(|(&t, &l)| (t, l.min(y_max).max(y_min))),
                SINGLE_COLOR.stroke_width(1),
            ))?
            .label(format!("{} (μ={:.3})", data.label, ln.mean))
