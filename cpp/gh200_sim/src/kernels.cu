...
// ===========================================================================
// SPLIT EXECUTION PATH (--split)
//
// Everything above this line is the fused path and is UNTOUCHED. The two
// kernels below are additive: they reuse wrapi(), s_other(), class_containing()
// and class_of() as-is, so there is exactly one copy of the exact-subtraction
// rule, the class-containment rule and the wrap.
//
// ---- What moves where -----------------------------------------------------
//   k_step_rhs   P0  scalar setup + run-and-tumble + zero ring
//                P1  cp.async strip pipeline (phi ONLY) + exact Ix/Iy
//                P1b fp64 fixed-order reduction -> v_n
//                P2  RHS sweep, phi^{n+1} written STRAIGHT TO GLOBAL through
//                    the same shifted-store / recentring map the fused P3 uses
//   k_step_post  P3  S scatter for step n+1, moments, bbox, phi_max
//                P3b fp64 fixed-order reduction + shape-class bookkeeping
//
// ---- S is read from global ------------------------------------------------
// S needs no stencil -- it enters the RHS and the Ix/Iy integrand pointwise --
// so the two sites that need it read A.S_rd directly. That costs a second pass
// over S (the fused kernel staged it once in smem and read it twice), which is
// affordable: S is L2-resident up to ~N=1600 and `--no-l2` measured neutral
// (135.13 vs 135.34 us at N=396), i.e. S traffic is not on the critical path.
//
// ---- Why the parity / rotation argument still holds ------------------------
// The invariant that makes `q_S - q_of(phi_n)` an exact, provably non-negative
// uint32 subtraction is: the buffer step n reads is byte-for-byte the buffer
// step n-1 scattered from, under the same rect->global map and the same q_of().
// The split preserves it without any change to the bookkeeping:
//
//   k_step_rhs (slot s)  writes phi^{n+1} into A.phi_out = d_phi_[pout]
//   k_step_post(slot s)  scatters q_of() FROM d_phi_[pout] into A.S_sc
//   k_step_rhs (slot s+1) has pin' = pout, so A.phi_in == d_phi_[pout]
//                         and A.S_rd == that same S_sc
//
// so the field that was scattered and the field that is read back are the same
// bytes, and the rect origin used by both is A.cell[n].gx0/gy0 -- which
// k_step_rhs publishes in P0 and k_step_post then reads. The S triple buffer
// rotates exactly as before: step n reads S[n%3], k_step_post scatters into
// S[(n+1)%3], and k_step_rhs zero-fills S[(n+2)%3]. Nothing else touches those
// buffers between the two launches, and the launches are stream-ordered, so the
// kernel boundary supplies all the ordering the split needs. NO parity fix-up
// was required.
//
// ---- Which kernel writes which CellState field -----------------------------
// The one real restructuring. The fused P3b wrote the whole record at the end;
// here the record is written in two halves, and the split is chosen so that
// k_step_post needs NO side channel and NO duplicate of P0's decision logic:
//
//   k_step_rhs  (in P0, right after the decision):
//       gx0, gy0, cls, cls_written[parity_out], theta, promote_ctr,
//       shift_ctr, tumble_ctr, cell_cls[n]        + vx, vy, Ix, Iy in P1b
//   k_step_post (in P3b):
//       V, Cx, Cy, perim, bb_*, phi_max
//
// k_step_post therefore reconstructs the destination window from cell_cls[n]
// (= dcls) and the rect origin from gx0/gy0 (= gx0n/gy0n) -- both already
// published -- and needs nothing else. P0's hysteresis/tumble/recentring logic
// exists in exactly one place per path.
//
// The record is transiently half-updated BETWEEN the two launches. Nothing
// reads it there: the only readers are k_morton_sort, k_pack_traj,
// k_verify_cells, print_line() and dump_state(), all of which run at step
// boundaries, and the pair is always launched together (also inside the graph
// body). k_step_rhs itself reads A.cell[n] into a local copy in P0 BEFORE it
// writes any of it back.
// ===========================================================================

// ---------------------------------------------------------------------------
// process_cell_rhs<CLS> -- kernel A's per-cell body.
//
// P0/P1/P1b are the fused kernel's phases with S_s removed; P2 is the fused
// sweep with the staging store replaced by the fused P3's destination map.
// The reduction is over kSplitWarps slots in ascending warp index, i.e. still
// a FIXED order determined entirely by compile-time constants.
// ---------------------------------------------------------------------------
template <int CLS>
__device__ void process_cell_rhs(int n, const StepArgs& A, char* smem,
                                 unsigned long long step)
{
    constexpr int WX  = kClasses[CLS].wx;
    constexpr int WY  = kClasses[CLS].wy;
    ...
    constexpr int PX  = phi_pitch(WX);
    constexpr int NS  = WY / kStripRows;
    constexpr int RB  = (WY + kSplitWarps - 1) / kSplitWarps;
    constexpr int kRing = 2 * (WX + 2) + 2 * WY;

    // The split path accepts the STAGED classes only. Instantiating this for a
    // non-staged class is a compile error rather than a 46,272 B shared-memory
    // overrun at runtime -- k_step_rhs's `default:` refusal is the runtime half
    // of the same rule, and this is the half that cannot be forgotten.
    static_assert(class_stages_S(CLS),
                  "process_cell_rhs must not be instantiated for a class that "
                  "does not stage S: k_step_rhs requests kSmemRhsBytes, which "
                  "is sized for the staged classes only");
    static_assert(class_smem_rhs(WX, WY) <= kSmemRhsRaw,
                  "this shape class needs more phi_s than k_step_rhs requests");
    static_assert(RB * kSplitWarps >= WY,
                  "P2's row bands must cover the whole rect");
    static_assert(WY % kStripRows == 0, "P1's strips must tile the rect exactly");
    ...
    float*    bcf   = reinterpret_cast<float*>(smem + kRedBytes);
    float*    phi_s = reinterpret_cast<float*>(smem + kScalarBytes);
    // No S_s. That is the entire point of this kernel.
    ...
    // =====================================================================
    // P0 -- identical to the fused P0 (same decisions, same broadcast words),
    ...
    if (tid == 0) {
        ...
        // --- per-cell coefficients (all from params.cuh via StepArgs) -----
        const float gam  = cs.gamma;
        const float dwC  = A.bulk_scale * gam;
        const float volC = (float)(A.vol_scale * (A.A0 - cs.V));
        ...
        const int gx0n = wrapi(cs.gx0 + sx, A.L);
        const int gy0n = wrapi(cs.gy0 + sy, A.L);
        const int cls_changed = (dcls != (int)cs.cls_written[A.parity_out]);
        ...
        // --- publish the geometry / RNG half of the CellState update ------
        // Safe here: `cs` above is a whole-record local copy taken before any
        // of this, and nothing later in THIS kernel reads these fields back
        // (P1/P2 take the OLD origin from bci[2]/bci[3]). Publishing in P0
        // rather than at the end also means k_step_post needs no side channel:
        // cell_cls[n] and gx0/gy0 are exactly the destination geometry.
        CellState* csw = &A.cell[n];
        csw->gx0 = gx0n;  csw->gy0 = gy0n;
        csw->cls = (uint8_t)dcls;
    ...
    for (int i = tid; i < kRing; i += kSplitBlockThreads) {
        if (i < WX + 2) {
            phi_s[(kPhiPadLeft - 1) + i] = 0.0f;                        // row -1
        } else if (i < 2 * (WX + 2)) {
            const int k = i - (WX + 2);
    ...
    // =====================================================================
    // P1 -- strip-pipelined load (phi only now) + exact Ix/Iy.
    // =====================================================================
    auto issue_strip = [&](int s) {
        constexpr int kChunksPerRow = WX / 4;                 // 16 B chunks
        constexpr int kPhiUnits     = kStripRows * kChunksPerRow;
        for (int u = tid; u < kPhiUnits; u += kSplitBlockThreads) {
            const int cch = u % kChunksPerRow;
            const int y   = s * kStripRows + u / kChunksPerRow;
            __pipeline_memcpy_async(
                &phi_s[(y + 1) * PX + kPhiPadLeft + cch * 4],
    ...
    for (int s = 0; s < NS; ++s) {
        ...
        const int ybase = s * kStripRows;
        for (int idx = tid; idx < kStripRows * WX; idx += kSplitBlockThreads) {
            const int i = idx % WX;
            const int y = ybase + idx / WX;
            const float* p = &phi_s[(y + 1) * PX + kPhiPadLeft + i];
            const float c  = p[0];
            const float pE = p[1],  pW = p[-1];
            const float pN = p[PX], pS = p[-PX];
            // S straight from global: the same word the fused kernel staged in
            // S_s[y*WX+i], under the identical rect -> global map.
            int ggy = gy0 + y;
            if (ggy >= A.L) ggy -= A.L;
            const int ggx = (i < split) ? (gx0 + i) : (i - split);
            const float So = s_other(A.S_rd[(size_t)ggy * A.P + ggx], c, A.flags);
            const float gx = 0.5f * (pE - pW);
            const float gy = 0.5f * (pN - pS);
            aIx += (double)(c * gx * So);
            aIy += (double)(c * gy * So);
    ...
    // ---- P1b: fp64 warp butterfly, then a fixed-order serial sum over the
    // kSplitWarps warp slots in ascending warp index.
#pragma unroll
    for (int d = 16; d > 0; d >>= 1) {
        aIx += __shfl_down_sync(0xFFFFFFFFu, aIx, d);
        aIy += __shfl_down_sync(0xFFFFFFFFu, aIy, d);
    ...
    // The only place a tile is ever zeroed: the destination window of the new
    // class does not contain the old one, so the residue outside it would break
    // invariant I1. Hoisted ahead of the P2 stores (the fused path does it at
    // the top of P3, i.e. also before its stores). __syncthreads() is a block
    // memory fence as well as a barrier, so the zeros are visible to the stores
    // that follow. Fires only on a genuine class change.
    if (bci[7]) {
        for (int i = tid; i < kTileArea; i += kSplitBlockThreads) tile_out[i] = 0.0f;
        __syncthreads();
    }

    // =====================================================================
    // P2 -- RHS sweep, 3x3 rolling window in registers, and the store goes
    // STRAIGHT TO GLOBAL through the fused P3's destination map:
    //
    //     destination (a,b) <- source (i,j) = (a+sx, b+sy)   when in the rect
    //     destination (a,b) <- 0.0f                          otherwise
    //
    // so here, walking the SOURCE, pixel (x,y) lands at (a,b) = (x-sx, y-sy)
    // whenever that is inside [0,dwx) x [0,dwy). The complementary set -- every
    // destination pixel whose source lies outside the rect -- is filled with
    // 0.0f by the frame pass below, using the fused predicate verbatim. The two
    // sets partition the destination window exactly, so every destination pixel
    // is written exactly once and the global result is identical to the fused
    // path's.
    //
    // Plain stores, not __stcs: k_step_post re-reads these bytes immediately,
    // so evict-first ("streaming") is exactly the wrong hint here.
    // =====================================================================
    {
        const float gam  = bcf[8];
        const float dwC  = bcf[9];
        ...
        if (y0 < WY) {
            ...
            for (int xb = 0; xb < WX; xb += 32) {
                ...
                const int ggx = (x < split) ? (gx0 + x) : (x - split);
                const int a   = x - sx;
                const bool ain = ((unsigned)a < (unsigned)dwx);
                const float* p = &phi_s[y0 * PX + kPhiPadLeft + x];  // row y0-1
                float sW = p[-1], sC = p[0], sE = p[1];
                p += PX;                                             // row y0
                float cW = p[-1], cC = p[0], cE = p[1];
                for (int y = y0; y < y1; ++y) {
                    ...
                    int ggy = gy0 + y;
                    if (ggy >= A.L) ggy -= A.L;
                    const float So = s_other(A.S_rd[(size_t)ggy * A.P + ggx],
                                             cC, A.flags);
                    const float rhs = gam * lap
                                    - dwC * (cC * (1.0f - cC) * (1.0f - 2.0f * cC))
                                    + volC * cC
                    ...
                    const float pnew = cC + dtf * rhs;
                    const int b = y - sy;
                    if (ain && (unsigned)b < (unsigned)dwy)
    ...
    // ---- frame pass: destination pixels with no source pixel get 0.0f. The
    // predicate is the fused P3's (jin && iin), unmodified, so the two paths
    // agree on which pixels are zero. Disjoint from the P2 stores above, so no
    // barrier is needed between them.
    for (int b = warp; b < dwy; b += kSplitWarps) {
        const int j = b + sy;
        const bool jin = ((unsigned)j < (unsigned)WY);
        for (int a = lane; a < dwx; a += 32) {
            const int i = a + sx;
...
// ---------------------------------------------------------------------------
// k_step_rhs -- kernel A. Same persistent-CTA / work-cursor structure as
// k_step, same phase C, same step-counter publish.
// ---------------------------------------------------------------------------
__global__ __launch_bounds__(kSplitBlockThreads, kSplitRhsCtasPerSm)
void k_step_rhs(PF_GRID_CONSTANT const StepArgs A)
{
    extern __shared__ uint4 smem_raw_rhs[];
    char* smem = reinterpret_cast<char*>(smem_raw_rhs);
    int* ctrl = reinterpret_cast<int*>(smem + kRedBytes);

    const int tid = (int)threadIdx.x;

    // ---- Phase C: clear-ahead of S[(step+2)%3]. Unchanged from the fused
    // kernel, and still race-free: this kernel neither reads nor scatters into
    // that buffer, k_step_post scatters into a different one, and the kernel
    // boundary orders it against the launch that will use it.
    if (A.clear_ahead_words > 0ull) {
        uint4* dst = reinterpret_cast<uint4*>(A.S_cl);
        const size_t nvec   = (size_t)(A.clear_ahead_words >> 2);
        const size_t stride = (size_t)gridDim.x * (size_t)kSplitBlockThreads;
        const uint4 z = make_uint4(0u, 0u, 0u, 0u);
        for (size_t i = (size_t)blockIdx.x * kSplitBlockThreads + tid; i < nvec;
             i += stride)
            __stcs(&dst[i], z);
    }

    // Cursor reset for the NEXT launch and the device step counter publish stay
    // in kernel A only: k_step_post reads *A.step_rd (the slot the PREVIOUS
    // launch wrote, which kernel A does not touch -- it writes *A.step_wr, the
    // other slot) so both kernels of a step see the same step number.
    if (blockIdx.x == 0 && tid == 0) {
        *A.cursor_clear = 0ull;
        *A.step_wr = *A.step_rd + 1ull;
    }
    ...
    for (;;) {
        ...
        if (cls < 0) break;
        const int n = ctrl[kBcastCtrlN];
        // The split path REFUSES kClassLarge, loudly and by construction.
        //
        // process_cell_rhs<kClassLarge> would need
        // class_smem_rhs(192,192) = 157,376 B of phi_s, but the launch requests
        // kSmemRhsBytes = 111,104 B (sized for the classes below, so that two
        // CTAs fit one SM -- the only reason this path exists). Instantiating it
        // would overrun the CTA's shared memory by 46,272 B. Raising the request
        // instead would pin k_step_rhs at 1 CTA/SM and the path would buy
        // nothing. static_assert'd in params.cuh at kSmemRhsRaw.
        //
        // NOTE: promotion INTO the large class still works here -- the
        // destination geometry is runtime and P2 writes straight to global, so
        // the promoting step is handled correctly and k_step_post reads the new
        // window fine. It is the step AFTER, when this switch is handed cls==4,
        // that cannot run. The cell is skipped, the flag is counted, and
        // report_flags() declares the run INVALID.
        static_assert(kNumClasses == 5,
                      "k_step_rhs's dispatch switch must enumerate every class "
                      "it accepts and refuse the rest");
        switch (cls) {
            case kClassRound: process_cell_rhs<kClassRound>(n, A, smem, step); break;
            case kClassWide:  process_cell_rhs<kClassWide >(n, A, smem, step); break;
            case kClassTall:  process_cell_rhs<kClassTall >(n, A, smem, step); break;
            case kClassBig:   process_cell_rhs<kClassBig  >(n, A, smem, step); break;
            default:                       // kClassLarge, or a corrupt class id
                // Counter only -- deliberately NOT a device printf. vprintf has
                // an ABI call cost that ptxas would have to budget registers
                // for across the whole kernel, and k_step_rhs is pinned at 64
                // by __launch_bounds__(512, 2); a spill induced by a cold error
                // path would show up in the startup dump as exactly the kind of
                // false alarm the spill gate exists to catch. The host reports
                // it instead: Sim::fatal_flag_set() checks this counter at
                // every print interval and stops the run.
                if (tid == 0) atomicAdd(&A.flags[FLAG_CLASS_UNSUPPORTED], 1u);
                // process_cell_rhs ends with a __syncthreads() that protects
                // ctrl from the next iteration's tid-0 write; the skip path has
                // to supply it too. cls is a broadcast word, so this is uniform.
                __syncthreads();
                break;
        }
    }
...
// ---------------------------------------------------------------------------
// k_step_post -- kernel B. P3 + P3b, one CTA per cell, grid = N.
//
// Block shape, deliberately: kSplitBlockThreads = 512, the SAME shape as
// kernel A. Two reasons, in order.
//   1. The moment reduction must be a fixed order, and the order is set by the
//      (warp, lane) -> pixel walk together with the serial sum over warp slots.
//      Using one block shape for the whole path means the split path has ONE
//      reduction geometry to reason about, and V/Cx/Cy/perim are reproducible
//      from the block shape alone.
//   2. 512 threads is the shape that lets ptxas keep a workable register budget
//      while stacking CTAs: at kSplitPostCtasPerSm = 3 the budget is
//      65536/(3*512) = 42 registers for 48 warps/SM (75%). Four CTAs would be
//      100% but at a 32-register budget, which four fp64 accumulators plus the
//      bbox/phi_max integer set are unlikely to hold without spilling -- and
//      spilling to buy occupancy has already backfired once on this code.
//
// Shared memory is the reduction region ONLY, declared statically (1024 B), so
// this kernel needs no cudaFuncSetAttribute opt-in and shared memory never
// limits its occupancy.
//
// It re-reads phi^{n+1} from global. That is the one extra HBM pass the split
// costs (3 rather than 2), traded for the doubled CTA count.
// ---------------------------------------------------------------------------
__global__ __launch_bounds__(kSplitBlockThreads, kSplitPostCtasPerSm)
void k_step_post(PF_GRID_CONSTANT const StepArgs A)
{
    __shared__ double red_s[kSplitWarps * kRedSlots];

    const int slot = (int)blockIdx.x;
    if (slot >= A.N) return;
    const int n = (int)A.perm[(size_t)slot];

    // Destination geometry and rect origin, both published by k_step_rhs's P0.
    const int dcls = (int)A.cell_cls[n];
    const ShapeClass dc = class_of(dcls);
    const int dwx = dc.wx,  dwy = dc.wy;
    const int dtx0 = dc.tx0, dty0 = dc.ty0;
    const int gx0n = A.cell[n].gx0;
    const int gy0n = A.cell[n].gy0;

    // Same cadence predicate, same step number: kernel A wrote *A.step_wr (the
    // other slot), so *A.step_rd still holds this step's number.
    const unsigned long long step = *A.step_rd;
    // (step + 1): must match the predicate in k_step / k_step_rhs exactly, or the
    // two split kernels disagree about which frames carry a perimeter. See the
    // note at the k_step site for why the frame is phi^{step+1}.
    const int fm = (A.full_moment_every > 0 &&
                    ((step + 1ull) % (unsigned long long)A.full_moment_every) == 0ull);

    const float* __restrict__ tile = A.phi_out + (size_t)n * kTileArea;
    ...
    // =====================================================================
    // P3 -- S scatter for step n+1 + next step's moments and support bbox.
    // pn is read back from the stored frame instead of from the staged rect,
    // so it is bit-for-bit the value the fused P3 held.
    // =====================================================================
    double aV = 0.0, aCx = 0.0, aCy = 0.0, aPer = 0.0;
    int blox = dwx, bhix = -1, bloy = dwy, bhiy = -1, pmaxb = 0;
    ...
    for (int b = warp; b < dwy; b += kSplitWarps) {
        int gy = gy0n + b;
        if (gy >= A.L) gy -= A.L;
        // Derived from the __restrict__ `tile`, so it is a "based on" pointer
        // and must NOT be __restrict__ itself -- two restrict pointers to the
        // same object would be a violation.
        const float* row = &tile[(size_t)(dty0 + b) * kTilePitch + dtx0];
        for (int a = lane; a < dwx; a += 32) {
            ...
            if (fm) {
                // |grad phi| of the STORED frame, clamped at the destination
                // window with the I1 zero ring -- consistent with how the
                // Laplacian and the Ix/Iy gradients are taken everywhere else.
                //
                // This is the ONE place where the split path is not bitwise
                // identical to the fused path, and it is a genuine information
                // loss, not an oversight -- flagged rather than papered over.
                // The fused P3 reads these four neighbours out of the staged
                // SOURCE rect, so on the border it can pick up a source pixel
                // that lies OUTSIDE the destination window and is therefore
                // never stored anywhere: at a == dwx-1 when dwx+sx < WX (any
                // sx < 0 on an ordinary step), and symmetrically at a == 0 when
                // sx > 0, and the same on b/sy. The split path has no such pixel
                // to read and uses 0, exactly as the I1 ring does.
                //
                // Affected: `perim` ONLY -- pn, V, Cx, Cy, the bbox, phi_max and
                // the scatter all use pn alone and are bit-identical. It bites
                // only on full-moment steps, only on the 1-px border of the
                // window, where phi < kSupportEps unless FLAG_SUPPORT_CLIP is
                // already firing. Bound: ~1e-5 absolute per affected pixel
                // against a perimeter of order 2*pi*R ~ 300.
                float e = 0.0f, w = 0.0f, nn = 0.0f, ss = 0.0f;
                if (a + 1 < dwx) e  = row[a + 1];
                if (a - 1 >= 0)  w  = row[a - 1];
                if (b + 1 < dwy) nn = tile[(size_t)(dty0 + b + 1) * kTilePitch
                                           + dtx0 + a];
                if (b - 1 >= 0)  ss = tile[(size_t)(dty0 + b - 1) * kTilePitch
                                           + dtx0 + a];
                const float pgx = 0.5f * (e - w), pgy = 0.5f * (nn - ss);
                aPer += (double)sqrtf(pgx * pgx + pgy * pgy);
            }
...
#pragma unroll
    ...
    if (tid == 0) {
        ...
        // The geometry / RNG half of this record was written by k_step_rhs's P0.
        CellState* cs = &A.cell[n];
        cs->V = sV;  cs->Cx = sCx;  cs->Cy = sCy;
        if (fm) cs->perim = sPer;
        cs->bb_lo_x = Blox;  cs->bb_hi_x = Bhix;
...
// k_step_rhs asks for 110,464 B of DYNAMIC shared memory, far over the 48 KB
// default, so it needs its own opt-in -- the attribute is per-kernel, it is not
// inherited from k_step's. Carveout 100 is required as well as sufficient:
// two 110,464 B CTAs need 220,928 B, which only the 228 KB carveout provides.
//
// k_step_post deliberately appears nowhere here. Its 1,024 B of shared memory
// is STATIC and under the 48 KB static cap, so it must not (and need not) opt
// in to anything; its carveout is left at the default so the driver is free to
// give it the larger L1 that its global re-read of phi wants.
void configure_split_smem() {
    cudaFuncSetAttribute(reinterpret_cast<const void*>(k_step_rhs),
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         kSmemRhsBytes);
    cudaFuncSetAttribute(reinterpret_cast<const void*>(k_step_rhs),
                         cudaFuncAttributePreferredSharedMemoryCarveout, 100);
...
// ---------------------------------------------------------------------------
// Split path: two stream-ordered launches per step.
//
// Graph capture needs nothing special. Both launches go on the captured stream
// with an implicit dependency between them, so a captured 6-step body simply
// contains 12 kernel nodes instead of 6 and replays exactly like the fused one.
// kGraphBody stays at 6 -- it is still lcm(2 phi parities, 3 S rotation slots),
// which is a property of the argument rotation, not of the launch count.
//
// The same L2 access-policy window is applied to both kernels. It is centred on
// S_rd (and, when the pair happens to be contiguous, S_sc), so it is useful to
// k_step_rhs and merely inert for k_step_post, which touches only S_sc and phi.
// `--no-l2` measured neutral at N=396, so this is not worth a second policy.
// ---------------------------------------------------------------------------
                       ...
                       const void* l2_base, size_t l2_bytes, float l2_hit_ratio)
{
    ...
    cfg_a.gridDim = dim3((unsigned)grid, 1, 1);
    cfg_a.blockDim = dim3((unsigned)kSplitBlockThreads, 1, 1);
    cfg_a.dynamicSmemBytes = (size_t)kSmemRhsBytes;
    cfg_a.stream = stream;
    cfg_a.attrs = nattr ? attr : nullptr;
    cfg_a.numAttrs = (unsigned)nattr;
    cudaLaunchKernelEx(&cfg_a, k_step_rhs, A);

    // One CTA per cell. No work cursor: every cell is independent here and the
    // scatter is an integer atomicAdd, so the visit order is irrelevant to the
    // result. perm is still honoured so the two kernels traverse cells in the
    // same (Morton, when enabled) order and share L2 locality.
    cudaLaunchConfig_t cfg_b = {};
    cfg_b.gridDim = dim3((unsigned)(A.N > 0 ? A.N : 1), 1, 1);
    cfg_b.blockDim = dim3((unsigned)kSplitBlockThreads, 1, 1);
    cfg_b.dynamicSmemBytes = 0;
    cfg_b.stream = stream;
    cfg_b.attrs = nattr ? attr : nullptr;
    cfg_b.numAttrs = (unsigned)nattr;
    cudaLaunchKernelEx(&cfg_b, k_step_post, A);
}
