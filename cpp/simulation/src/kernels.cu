...
// Layout:
//   1. Device helpers
//   2. k_scatter_S       — atomicAdd phi^2 into global S (init scatter only)
//   3. k_polar           — per-cell polarity update (RTP + ABP)
//   4. k_init_phi        — tanh init profile
//   5. k_initial_velocity — one-shot velocity reduce (init / resume)
//   6. k_rng_init        — curand state seeding
//   7. Cell migration kernels (multi-GPU slab decomposition)
//
// The per-step evolution (scatter + reduce + RHS + rebind) lives in step.cu
// as the single-pass fused kernel k_step; this file holds the init,
// polarity, and multi-GPU migration kernels.
//
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 3. k_polar — per-cell polarity update.
// ---------------------------------------------------------------------------
